...
{		
		...
		//define('BASE_PATH',APP_URL . 'static/tl/js/fckeditor/');
		//var_dump();
		//$basePath	= APP_URL . 'static/tl/js/fckeditor/';
        if(!isset($params['InstanceName']) || empty($params['InstanceName']))
        {
                $smarty->trigger_error('fckeditor: required parameter "InstanceName" missing');
        }

        static $base_arguments = 	array( 'BasePath' => PATH_FCKEDITOR );
		...
		$config_arguments['CustomConfigurationsPath'] = 'vncconfig.js?' . (strftime("%Y-%m-%d %H:%M:%S", time()));

        // Test if editor has been loaded before
        if(count($base_arguments) > 0)
        	$init = true;
        else
        	$init = false;
